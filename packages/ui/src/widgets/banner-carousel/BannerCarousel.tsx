...
import type { Breakpoint, DesktopMobile, Frame, NRCCarouselProps } from './types';
import clsx from 'clsx';
import { BCFrame } from './BCFrame';
...
export const BannerCarousel = ({
  ...
  noBlur,
  ariaLabel,
  controlsComponent = BCControl,
  willAutoPlayOutsideViewport,
  classNames,
}: NRCCarouselProps) => {
  const breakpointClass: DesktopMobile<string> = breakpoint
  ...
  return (
    <div
      className={cn('overflow-hidden w-full relative', classNames?.containerClassnames)}
      ref={containerRef}
    ...
    >
      <div
        className={clsx('transition-transform duration-500 ease-in-out', breakpointClass.mobile)}
      ...
      >
        {infiniteFrames.map((frame, i) => (
          <div
            ...
            className={cn('relative w-full flex-shrink-0', classNames?.frameClassnames)}
            aria-hidden={i !== index}
            inert={i !== index}
            style={{
          ...
          >
            ...
            <BCFrame
              ...
              noBlur={noBlur}
              imageClassnames={classNames?.imageClassnames}
            />
          </div>
        ))}
      </div>
      ...
      {controlsComponent({
        ...
        totalFrames: infiniteFrames.length - 2,
      })}
    </div>
  );
};
