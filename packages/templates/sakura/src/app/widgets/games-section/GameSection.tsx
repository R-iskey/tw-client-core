...
import { GamesList } from '@triple-win/template-sakura/app/widgets/games-section/GamesList';
...
export function GameSection(props: GameSectionProps) {
  ...
  return (
    <div>
      ...
      {data.map((section) => (
        <GamesList key={section.title} title={section.title} items={section.items} />
      ))}
    </div>
  );
}
